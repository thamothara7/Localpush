with open('popup.js', 'rb') as f:
    content = f.read()

target = b"""    const port = new URL(tab.url).port || '80';

    // New port \xe2\x80\x94 resolve repo
    if (port !== activePort) {
      activePort = port;
      ...
      if (repos[port]) {
        ...
        repo = null;
        setRepoPill('spin', `Detecting repo on :${port}\xe2\x80\xa6`);
...
replacement = b"""    const port = new URL(tab.url).port || '80';
...
# The previous replace failed because it didn't match the exact bytes.
# Wait, let's just do a string replace reading with errors='replace', but to save it properly, we just read and write using surrogateescape.
